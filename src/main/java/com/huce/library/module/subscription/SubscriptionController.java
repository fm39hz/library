...
public class SubscriptionController {
    ...
    public ResponseEntity<SubscriptionResponseDto> returnBook(@UserId Long userId, @PathVariable Long bookId) {
        CustomUserDetails user = (CustomUserDetails) userService.loadUserById(userId);
        return ResponseEntity.ok().body(new SubscriptionResponseDto(subscriptionService.returnBook(bookId, user.getUser().getId())));
    }
    ...
    @GetMapping("/renew")
    public ResponseEntity<PaymentResponseDto> renewSubscription(
            @UserId Long userId
